    ...
    public void run() {
        try {
            while (true) {
                ...
                System.out.println("hello123 " + data);
                switch (CMD) {
                    case "CMD_JOIN":
                    ...
                    case "CMD_CHATALL":
                        ...
                        break;
                    case "CMD_CHAT_TO_PERSON":
                        String chatPersonFrom = st.nextToken(); // công thức: CMD_CHAT_TO_PERSON 'chatPersonFrom' 'chatFor' 'mess'
                        String chatFor = st.nextToken();
                        String chatPerson_msg = "";
                        while (st.hasMoreTokens()) {
                            chatPerson_msg = chatPerson_msg + " " + st.nextToken();
                        }
                        String chatPerson_content = chatPersonFrom + " " + chatPerson_msg;
                        for (int x = 0; x < main.clientList.size(); x++) {
                            if (main.clientList.elementAt(x).equals(chatFor)) {
                                try {
                                    Socket tsoc2 = (Socket) main.socketList.elementAt(x);
                                    DataOutputStream dos2 = new DataOutputStream(tsoc2.getOutputStream());
                                    dos2.writeUTF("CMD_PRIVATE " + chatPerson_content);
                                    break;
                                } catch (IOException e) {
                                    main.appendMessage("[CMD_CHAT_TO_PERSON]: " + e.getMessage());
                                }
                            }
                        }
                        main.appendMessage("[CMD_CHAT_TO_PERSON]: " + chatPersonFrom + " tới " + chatFor + ": " + chatPerson_msg);
                        break;
